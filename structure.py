    ...
    def __call__(self, input):
        with tf.variable_scope(self.name):
            ...
            if self.image_size <= 128:
                res_output = layers.n_Rk(d128, reuse=self.reuse, n=6)
            else:
                res_output = layers.n_Rk(d128, reuse=self.reuse, n=9)
...
class Discriminator:
    ...
    def __call__(self, input):
        with tf.variable_scope(self.name):
            ...
            output = layers.last_layer(C512, reuse=self.reuse, use_sigmoid=self.use_sigmoid, name='output') 
